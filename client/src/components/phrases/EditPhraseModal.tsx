...
const EditPhraseModal: React.FC<EditPhraseModalProps> = ({ open, onClose, onSuccess, phrase }) => {
  ...
  // Reset form with phrase data when phrase or open changes
  useEffect(() => {
    if (open && phrase) {
      const exampleSentences =
        phrase.exampleSentences.length > 0
          ? phrase.exampleSentences.map((sentence) => ({ value: sentence }))
          : [{ value: '' }];
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await apiService.updatePhrase(phrase._id, phraseData);
