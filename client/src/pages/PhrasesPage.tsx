...
import { usePhraseTopics, useDeletePhraseTopic } from '../hooks/usePhrases';
import { useDebounce } from '../hooks/useDebounce';
import { PhraseTopic } from '../types';
import { useApp } from '../context/AppContext';
import CreateTopicModal from '../components/phrases/CreateTopicModal';
import ImportTopicModal from '../components/phrases/ImportTopicModal';
...
const PhrasesPage: React.FC = () => {
  ...
  const [topicToDelete, setTopicToDelete] = useState<PhraseTopic | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [orderBy, setOrderBy] = useState<'name' | 'phraseCount' | 'updatedAt'>('name');
  ...
  const deleteTopicMutation = useDeletePhraseTopic();
  ...
  const handleTopicClick = (topicId: string) => {
    navigate(`/phrases/${topicId}`);
  };

  const handleSort = (column: 'name' | 'phraseCount' | 'updatedAt') => {
  ...
  const handleDeleteClick = (e: React.MouseEvent, topic: PhraseTopic) => {
    e.stopPropagation();
    ...
    <Container maxWidth="lg">
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary" paragraph>
          Select a topic to start learning phrases
        </Typography>
      </Box>
              ...
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="center">
                    <TableSortLabel
                      active={orderBy === 'phraseCount'}
                      direction={orderBy === 'phraseCount' ? order : 'asc'}
                      onClick={() => handleSort('phraseCount')}
                    >
              ...
              <TableBody>
                {topics.map((topic) => (
                  ...
                  >
                    ...
                    <TableCell align="center">
                      {topic.phraseCount} {topic.phraseCount === 1 ? 'phrase' : 'phrases'}
                    </TableCell>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            ...
            <br />
            <strong>
              This will also delete all {topicToDelete?.phraseCount || 0} phrases in this topic.
            </strong>
            <br />
